...
if length(col) == length(mult)
	...
	for i = [1:length(col)]
		...
		data.rate = fs(i);
		[ad, S, err, tau] = allan_overlap(data, horzcat(reshape([1:0.1:9]'.*10.^[0:round(log10(length(data.freq)))-1],1,[]), 10^(round(log10(length(data.freq)))-1))./data.rate, strcat(strsplit(filename, '/'){end}, num2str(i)), 0);
